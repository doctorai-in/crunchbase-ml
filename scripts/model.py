...
class FundImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.clf.fit(rounds_funds.rounds.to_numpy().reshape(shape), rounds_funds.funded.to_numpy().reshape(shape))
        return self
    ...
    def transform(self, X):
        ...
        prediction = self.clf.predict(frame[null_funded].total_rounds.to_numpy().reshape(null_funded_shape))
        frame.loc[null_funded, 'average_funded'] = prediction.ravel()
        transformed = frame.average_funded.to_numpy().reshape(total_shape)
        return transformed
...
class ParticipantsImputer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        features = funds_participants.average_funded.to_numpy().reshape(shape)
        ground_truth = funds_participants.average_participants.to_numpy().reshape(shape)
    ...
    def transform(self, X):
        ...
        prediction = self.clf.predict(frame[null_participants].average_funded.to_numpy().reshape(null_funded_shape))
        frame.loc[null_participants, 'average_participants'] = prediction.ravel()
        transformed = frame.average_participants.to_numpy().reshape(total_shape)
        return transformed
...
class ModelBuilder:
    ...
    def read_data(self, data_path, train_path, test_path):
        ...
        self.Y_train = train_data.is_acquired.to_numpy()
        self.X_test = test_data.drop(['company_id', 'is_acquired'], axis=1)
        self.Y_test = test_data.is_acquired.to_numpy()
